...
// MARK: - Auth Verwaltung

@MainActor
final class AuthVerwaltung: ObservableObject {
    @Published var token: String? = Schluesselbund.laden()
    @Published var fehlermeldung: String?


    private let api = APIClient(basisURL: URL(string: "http://100.83.30.52:8000")!)

    func anmelden(firmenID: String, benutzername: String, passwort: String) async {
        fehlermeldung = nil
        do {
            let antwort = try await api.anmelden(firmenID: firmenID, benutzername: benutzername, passwort: passwort)
            Schluesselbund.speichern(antwort.token)
            token = antwort.token
        } catch {
            fehlermeldung = (error as? LocalizedError)?.errorDescription ?? "Login fehlgeschlagen"
        }
    }
    ...
    func artikelLaden() async throws -> [Artikel] {
